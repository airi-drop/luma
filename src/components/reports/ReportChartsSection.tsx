...
function ChartSurface({ children, height = "h-52" }: { children: ReactNode; height?: string }) {
  return (
    <div
      className={[
        height,
        "overflow-hidden rounded-[20px] border border-[var(--border-soft)] bg-[linear-gradient(180deg,rgba(var(--overlay-base-rgb),0.5),rgba(var(--overlay-base-rgb),0.8))] px-2 py-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-sm",
      ].join(" ")}
    >
      {children}
    </div>
...
const chartTabs: ReportChartTabMeta[] = [
  {
    id: "category",
    label: "Kategori",
    title: "Ringkasan kategori",
    subtitle: "Pilih satu tab dan lihat visualnya di panel yang sama.",
    helper: "Kategori mana yang paling banyak kepakai bulan ini.",
  },
  {
    id: "trend",
    label: "Tren",
    ...
    subtitle: "Pilih satu tab dan lihat visualnya di panel yang sama.",
    helper: "Naik turunnya pengeluaran harian di bulan ini.",
  },
  {
    id: "budget",
    label: "Budget",
    ...
    subtitle: "Pilih satu tab dan lihat visualnya di panel yang sama.",
    helper: "Bandingin limit budget dan pemakaian tanpa buka card lain.",
  },
];
...
export function ReportChartsSection({ reportData }: ReportChartsSectionProps) {
  ...
  return (
    <Card className="px-4 py-4" title="Panel laporan" subtitle={activeTabMeta.subtitle}>
      <div className="rounded-[22px] border border-[var(--border-soft)] bg-[linear-gradient(180deg,rgba(var(--overlay-base-rgb),0.18),rgba(var(--overlay-base-rgb),0.08))] p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]">
        <div className="grid grid-cols-3 gap-1 rounded-[18px] bg-[var(--bg-card-soft)]/74 p-1">
          {chartTabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={[
                  "min-h-[34px] rounded-[14px] px-2.5 text-[11px] font-semibold transition-[color,background-color,border-color,box-shadow,transform] duration-150",
                  isActive
                    ? "bg-[linear-gradient(180deg,rgba(var(--overlay-glow-primary),0.24),rgba(var(--overlay-glow-secondary),0.12))] text-[var(--text-primary)] shadow-[0_8px_16px_rgba(122,90,72,0.12),inset_0_-2px_0_var(--accent-primary)]"
            ...
            <>
              <ChartSurface height="h-44">
                <ResponsiveContainer height="100%" width="100%">
                  <PieChart>
                    ...
                    <Tooltip content={<ChartTooltip labelPrefix="Kategori" />} cursor={false} />
                  </PieChart>
                </ResponsiveContainer>
              </ChartSurface>
              <div className="mt-2 grid gap-1.5">
                {reportData.categoryBreakdown.slice(0, 5).map((item) => (
                  <div
                    key={item.category}
                    className="flex items-center justify-between gap-2 rounded-2xl bg-[var(--bg-card-soft)] px-2.5 py-1.5 text-[11px]"
                  >
          ...
          {activeTab === "trend" ? (
            <>
              ...
              <p className="mt-2 px-1 text-[11px] leading-4 text-[var(--text-secondary)]">
                Pola harian di {reportData.monthLabel.toLowerCase()} jadi lebih cepat kebaca dalam
                panel yang sama.
              </p>
            </>
          ) : null}
            ...
            <>
                ...
                <ResponsiveContainer height="100%" width="100%">
                  <BarChart data={reportData.budgetComparison}>
                    ...
                    <YAxis
                      ...
                      width={42}
                    />
                    <Tooltip content={<ChartTooltip />} cursor={{ fill: "rgba(122,90,72,0.06)" }} />
              ...
              <div className="mt-2 grid gap-1.5">
                ...
                {reportData.budgetComparison.length === 0 ? (
                  <p className="px-1 text-[11px] leading-4 text-[var(--text-secondary)]">
                    Begitu budget bulanan atau per kategori dipasang, perbandingannya muncul di
                    sini.
                  </p>
                ) : null}
              </div>
