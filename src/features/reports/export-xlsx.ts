...
function sanitizeSpreadsheetRow<TValue extends Record<string, unknown>>(row: TValue) {
...
export async function exportMonthlySpreadsheetXlsx(month: string) {
  ...
  const summaryRows = [
    sanitizeSpreadsheetRow({ Metrik: "Bulan", Nilai: reportData.monthLabel }),
    sanitizeSpreadsheetRow({ Metrik: "Total spending", Nilai: reportData.totalSpending }),
    sanitizeSpreadsheetRow({
